...
docker run --user root:root --hostname $HOSTNAME --net=$NET --device=/dev/dri:/dev/dri --privileged -e "QT_X11_NO_MITSHM=1" -e GAZEBO_MODEL_PATH=$GAZEBO_MODEL_PATH:/opt/ros/$ROS/share/wolf_gazebo_resources/models/ -e SHELL -e DISPLAY -e DOCKER=1 -e DISABLE_ROS1_EOL_WARNINGS=1 --name $CONTAINER_NAME \
        ...
        --device=/dev/ttyUSB0 \
        --volume="/tmp:/tmp:rw" \
        -it $IMAGE_NAME $SHELL -c "source /opt/ros/$ROS/setup.bash; . /usr/share/gazebo/setup.sh; ros2 launch wolf_controller wolf_controller_bringup.launch.xml world_name:=$WORLD_NAME robot_name:=$ROBOT_NAME robot_model:=$ROBOT_MODEL initial_xyz:=[0.0,0.0,0.0] initial_rpy:=[0.0,0.0,0.0]"
